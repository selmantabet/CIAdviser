...
from tkinter.ttk import * #New UI + progressbar.
import time #To put the program on sleep so that the progressbar could appear smoother.
import tkinter.messagebox #For info box displays.
import tkinter.simpledialog #For quick dialog inputs.
import matplotlib
...
# Version 1.2.0a [ALPHA], June 12th, 2020.
#
# GDAX API is now deprecated, so this may be the last major update depending on when the GDAX API would go offline.
#
# Changelog:
#
# [CODE]
...
# - Reworked password function.
# - Overhauled data fetch process.
#  ~ Granularity re-adjusted to 300 in response to the new GDAX API limit on granularity per request.
#  ~ Split single one-week requests to bursts of 7 in response to the new GDAX API limit on request frequency.
#
# [UI]
#
# - Revamped UI using ttk.
# - Default selection removed.
#
# ! ALPHA STAGE: Data fetch and plot fix underway. Currently very buggy.
...
class Mainw(Tk): #Main Window
    ...
    def TopUp(self): #This function prompts the user to enter an amount of money that would be added to their USD balance.
        ...
        while self.legalTopUp==False: #Keep on prompting the user until they provides a valid input.
            self.amt=tkinter.simpledialog.askfloat("Balance Top Up (US Dollars)","How much are you willing to top up?")
            if self.amt<0:
                tkinter.messagebox.showinfo("Illegal value.","Your top up value cannot be negative.\nInput zero if you wish to cancel.")
            else:
    ...
    def refresh(self): #This function fetches all the latest values and data via the GDAX API, and uses the received data to plot new graphs and display new trend values.
        ...
        self.progress=Progressbar(self.frame,orient="horizontal",mode="determinate")
        ...
        self.weeklooper = range(1,8) #This will be used in the for loops to help calculating date deltas.
        print("Receiving BTC data...")
        self.bdata = []
        for i in self.weeklooper:
            if (i == 6):
                self.btc24 = self.req.get_product_historic_rates("BTC-USD",start=(datetime.datetime.utcnow()-datetime.timedelta(days=8-i)).isoformat(),end=(datetime.datetime.utcnow()-datetime.timedelta(days=7-i)).isoformat(),granularity="300")
                self.bdata.extend(self.btc24)
                self.btc24 = (self.btc24[len(self.btc24)-2][1] + self.btc24[len(self.btc24)-2][2]) / 2 #24hr point for 24hr ratioing later.
            else:
                self.bdata.extend(self.req.get_product_historic_rates("BTC-USD",start=(datetime.datetime.utcnow()-datetime.timedelta(days=8-i)).isoformat(),end=(datetime.datetime.utcnow()-datetime.timedelta(days=7-i)).isoformat(),granularity="300")) #Maximum granularity for this specific time-frame, the server does not allow requests that result in anything more than ~400 data points.
        while 'message' in self.bdata:
            self.bdata.remove('message')
        ...
        self.edata = []
        for i in self.weeklooper:
            self.edata.extend(self.req.get_product_historic_rates("ETH-USD",start=(datetime.datetime.utcnow()-datetime.timedelta(days=8-i)).isoformat(),end=(datetime.datetime.utcnow()-datetime.timedelta(days=7-i)).isoformat(),granularity="300"))
        while('message' in self.edata):
            self.edata.remove('message')
        ...
        self.ldata = []
        for i in self.weeklooper:
            self.ldata.extend(self.req.get_product_historic_rates("LTC-USD",start=(datetime.datetime.utcnow()-datetime.timedelta(days=8-i)).isoformat(),end=(datetime.datetime.utcnow()-datetime.timedelta(days=7-i)).isoformat(),granularity="300"))
        while('message' in self.ldata):
            self.ldata.remove('message')
        ...
        if self.logs==[]:
            ...
            if self.totalchange>0: #Format the label according to the delta signs.
                self.totdeltadisp.config(text="$ "+str(round(self.totalchange,2)),foreground="dark green")
            elif self.totalchange<0:
                self.totdeltadisp.config(text="$ "+str(round(self.totalchange,2)),foreground="red")
            else:
                self.totdeltadisp.config(text="$ "+str(round(self.totalchange,2)))

            if self.BTCdelta>0:
                self.BTCdeltadisp.config(text="BTC: $ "+str(round(self.BTCdelta,2)),foreground="dark green")
            elif self.BTCdelta<0:
                self.BTCdeltadisp.config(text="BTC: $ "+str(round(self.BTCdelta,2)),foreground="red")
            else:
                self.BTCdeltadisp.config(text="BTC: $ "+str(round(self.BTCdelta,2)))
                
            if self.ETHdelta>0:
                self.ETHdeltadisp.config(text="ETH: $ "+str(round(self.ETHdelta,2)),foreground="dark green")
            elif self.ETHdelta<0:
                self.ETHdeltadisp.config(text="ETH: $ "+str(round(self.ETHdelta,2)),foreground="red")
            else:
                self.ETHdeltadisp.config(text="ETH: $ "+str(round(self.ETHdelta,2)))

                
            if self.LTCdelta>0:
                self.LTCdeltadisp.config(text="LTC: $ "+str(round(self.LTCdelta,2)),foreground="dark green")
            elif self.LTCdelta<0:
                self.LTCdeltadisp.config(text="LTC: $ "+str(round(self.LTCdelta,2)),foreground="red")
            else:
        ...
        self.BTC24hr=((self.BTCval-self.btc24)/self.btc24)*100 #Because 24 hours consists of ~58 data points. The difference between today's rates and yesterday's is ratioed.
        ...
        if self.BTC24hr>0 and self.BTC1wk>0: #If both trends are positive, then there is a steady increase in the value.
            self.BTCvaldisp.config(text="BTC: $"+str(self.BTCval)+" |  "+str(round(self.BTC24hr,3))+"% | "+str(round(self.BTC1wk,3))+"%",foreground="dark green")
        elif self.BTC24hr<0 and self.BTC1wk<0: #If both trends are negative, then there is a steady decrease in the value.
            self.BTCvaldisp.config(text="BTC: $"+str(self.BTCval)+" |  "+str(round(self.BTC24hr,3))+"% | "+str(round(self.BTC1wk,3))+"%",foreground="red")
        ...
        if self.ETH24hr>0 and self.ETH1wk>0:
            self.ETHvaldisp.config(text="ETH: $"+str(self.ETHval)+" |  "+str(round(self.ETH24hr,3))+"% | "+str(round(self.ETH1wk,3))+"%",foreground="dark green")
        elif self.ETH24hr<0 and self.ETH1wk<0:
            self.ETHvaldisp.config(text="ETH: $"+str(self.ETHval)+" |  "+str(round(self.ETH24hr,3))+"% | "+str(round(self.ETH1wk,3))+"%",foreground="red")
        else:
        ...
        if self.LTC24hr>0 and self.LTC1wk>0:
            self.LTCvaldisp.config(text="LTC: $"+str(self.LTCval)+" |  "+str(round(self.LTC24hr,3))+"% | "+str(round(self.LTC1wk,3))+"%",foreground="dark green")
        elif self.LTC24hr<0 and self.LTC1wk<0:
            self.LTCvaldisp.config(text="LTC: $"+str(self.LTCval)+" |  "+str(round(self.LTC24hr,3))+"% | "+str(round(self.LTC1wk,3))+"%",foreground="red")
        else:
    ...
    def showplot(self):
        ...
        if stat==1: #BTC Choice
            ...
            if self.firstrun==False: #To check if there was already a canvas that it should overwrite.
                ...
                canvas.draw() #Otherwise, this is the first run. Grid the canvas.
                canvas.get_tk_widget().grid(row=3,column=1,columnspan=10)
                self.firstrun=False
...
class Investment(Toplevel):
    ...
    def InvestAnalyze(self,choice,USDamount): #This function checks how much the user would earn within one week period based on the daily and weekly trends of the selected cryptocurrency.
        if float(USDamount)>self.balance: #Check for illegal inputs.
            tkinter.messagebox.showinfo("Invalid input.","You cannot invest with more than what you have.\nPlease lower the investment value or top up your USD Balance.")
            self.lift() #Raise the window back up after closing the message box.
        elif float(USDamount)<0:
            tkinter.messagebox.showinfo("Invalid input.","You cannot invest a negative amount of money.\nMake sure that the investment amount is a positive value.")
            self.lift()
        elif float(USDamount)==0:
            tkinter.messagebox.showinfo("Zero investment.","Your investment cannot be zero. You might as well not do anything.")
            self.lift()
        else:
            ...
            if choice==1: #BTC Choice
                ...
                if self.Potential-float(USDamount)<0: #Format according to numerical signs.
                    self.Potentialdisp.config(text="Net earning wihin one week:  $ "+str(self.Potential-float(USDamount)),foreground="red")
                else:
            ...
            #####################################   ANALYZING LTC INVESTMENT    #####################################
                ...
                self.receivedisp.config(text="You would receive:  "+self.received)
                if self.Potential-float(USDamount)<0:
                    self.Potentialdisp.config(text="Net earning wihin one week:  $ "+str(float(USDamount)-self.Potential),foreground="red")
                else:
                    self.Potentialdisp.config(text="Net earning wihin one week:  $ "+str(self.Potential-float(USDamount)))
    ...
    def InvestFunction(self,choice,USDamount): #Takes an amount from the user's USD balance and converts it to a cryptocurrency of the user's choice. Then it auto-saves and logs this activity.
        if USDamount=="" or USDamount==None:
            tkinter.messagebox.showinfo("Empty input!","Please enter a valid figure in the entry box.")
            self.lift()
        else:
            if self.Potential-float(USDamount)<0: #Check if the user is sure to invest even though it is predicted that he would be losing money.
                self.confirminv=tkinter.messagebox.askyesno("Confirm investment.","The potential earning for this investment seems to be negative.\nAre you sure of your decision?")
            else:
        ...
        if amount=="" or amount==None:
            ...
            if choice=="BTC":
                if float(amount)>AppWin.BTCbal:
                    tkinter.messagebox.showinfo("Invalid input.","You cannot cashout with more than what you have.\nPlease lower the cashout value.")
                    self.lift()
                elif float(amount)<0:
                    tkinter.messagebox.showinfo("Invalid input.","You cannot cashout a negative amount of money.\nMake sure that the cashout amount is a positive value.")
                    self.lift()
                elif float(amount)==0:
                    tkinter.messagebox.showinfo("Zero cashout.","Your cashout cannot be zero. You might as well not do anything.")
                    self.lift()
                else:
...
class LoginWindow(Tk): #Login Page, where the login process takes place.
    ...
    def LOGIN(self,username,password): #Takes the password, hashes it and checks if it is correct. If so, then it will load the user's data and transfer them to the Main Window.
        if os.path.exists(username+".txt"):
            ...
            checker=bcrypt.checkpw(bytes(password,'utf8'),bytes(target, 'utf8')) #Attempt to hash the provided password with the salt of the target password.
            if checker == True: #Compare if the hashed password matches the target hashed string.
                ...
                tkinter.messagebox.showinfo("Success!","Login successful! Welcome to the trading world!")
                ...
                tkinter.messagebox.showinfo("Wrong credentials","Wrong password, please try again.")
            
        else:
            tkinter.messagebox.showinfo("Wrong credentials","Username incorrect or does not exist.\nIf you are a new user, create an account before logging in.")
...
class AccCreation(Toplevel):
    ...
    def Create(self,username,password,passconf):
        if os.path.exists(username+".txt"): #Check if the username exists.
            tkinter.messagebox.showinfo("Username taken","Username already exists, enter a different username.")
            ...
            tkinter.messagebox.showinfo("Invalid input!","Make sure you have filled in all the fields!")
            self.lift()
        elif password!=passconf: #Check if the passwords match.
            tkinter.messagebox.showinfo("Invalid input!","Passwords do not match!\nMake sure that you have typed the passwords correctly.")
            self.lift()
            ...
            salt=bcrypt.gensalt()
            f.write(str(salt)+'\n')
            f.write(bcrypt.hashpw(bytes(password, 'utf8'), salt).decode("utf-8") +'\n')
            f.write("USDbal=0\n")
            f.write("BTCbal=0\n")
            f.write("ETHbal=0\n")
            f.write("LTCbal=0\n")
            f.close()
            tkinter.messagebox.showinfo("Creation success!","Account successfully created!\nYou may now login with your credentials.")
